...
func SolveBruteForce(instance vrp.VRPInstance, logger *logging.Logger) vrp.Solution {
	...
	permute(customerIDs, func(order []int) {
		findBestPartition(order, k, capacity, demandByID, dist, &best)
	})
...
func findBestPartition(order []int, numVehicles int, capacity int, demand []int, dist [][]float64, best *vrp.Solution) {
	n := len(order)
	if n == 0 {
		return
	}
	...
	findPartitionRec(order, assignment, 0, numVehicles, capacity, demand, dist, best)
}

func findPartitionRec(order []int, assignment []int, customerIdx int, numVehicles int, capacity int, demand []int, dist [][]float64, best *vrp.Solution) {
	if customerIdx == len(order) {
		evaluatePartition(order, assignment, numVehicles, capacity, demand, dist, best)
		return
	}

	for vehicle := 0; vehicle < numVehicles; vehicle++ {
		assignment[customerIdx] = vehicle
		findPartitionRec(order, assignment, customerIdx+1, numVehicles, capacity, demand, dist, best)
	}
}

func evaluatePartition(order []int, assignment []int, numVehicles int, capacity int, demand []int, dist [][]float64, best *vrp.Solution) {
